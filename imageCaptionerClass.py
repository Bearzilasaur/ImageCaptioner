...
class ImageCaptioner(tk.Tk):
    ...
    # get images from directory (user selects directory)
    def getDir(self):
        dirpath = filedialog.askdirectory(initialdir="./", title="Select Images Folder")
        images = [
            [path.join(dirpath, file), file]
        ...
        self.workingImage = self.imagepaths[self.imageIncrementer][1]
    ...
    def listImages(self, imagelist):
        ...
        for image in imagelist:
            if image[1] == self.workingImage:
                colour = "yellow"
            else:
                colour = "grey"
            label = tk.Label(self.dirframe, text=image[1], bg=colour)
            label.pack()
    ...
    def bake_caption(self):
        self.captiontext = (
            self.caption.get()
        )  # "1.0","end-1c") # these options make the returned string nicer
        # do work here!
        ...
        if self.imageIncrementer >= self.numberImages:
            print("END OF FILES")
            # checks
            print(f"\n\n\n\nself.imagepaths: {self.imagepaths}\n\n\n\n")
            for i in self.imagepaths:
